...
class InfluxDBConnection(DatabaseConnection):
    ...
    def __init__(self, config: ConnectionConfig):
        ...
        if InfluxDBClient is None:
            raise ImportError(
                "influxdb-client is required for InfluxDB support. Install it with: pip install influxdb-client"
            )
    ...
    async def connect(self) -> None:
        ...
        token = (
            self.config.password or self.config.extra.get("token", "")
    ...
    async def restore_backup(self, backup_path: str) -> None:
        ...
        with open(backup_path, "r") as f:
            ...
            self.write_api.write(bucket=self.config.database, record=points)

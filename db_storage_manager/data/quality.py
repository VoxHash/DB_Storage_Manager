...
class DataQualityAnalyzer:
    ...
    async def analyze_table_quality(self, table_name: str) -> Dict[str, Any]:
        ...
        # Get basic stats
        try:
            null_result = await self.database.execute_query(
                null_check_query, safe_mode=True
            )
            duplicate_result = await self.database.execute_query(
                duplicate_check_query, safe_mode=True
            )

            total_rows = (
                null_result.get("rows", [{}])[0].get("total_rows", 0)
                if null_result.get("rows")
                else 0
            )
            null_count = (
                null_result.get("rows", [{}])[0].get("null_count", 0)
                if null_result.get("rows")
                else 0
            )
            duplicate_count = (
                duplicate_result.get("rows", [{}])[0].get("duplicate_count", 0)
                if duplicate_result.get("rows")
                else 0
            )
            ...
            return {
                ...
                "issues": self._identify_issues(
                    null_count, duplicate_count, total_rows
                ),
            }
    ...
    def _identify_issues(
        self, null_count: int, duplicate_count: int, total_rows: int
    ) -> List[str]:

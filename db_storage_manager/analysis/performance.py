...
class QueryPerformanceAnalyzer:
    ...
    def get_query_statistics(self) -> Dict[str, Any]:
        ...
        successful_queries = [q for q in self.query_history if q.get("status") == "success"]
        
        if successful_queries:
            avg_time = sum(q.get("executionTime", 0) for q in successful_queries) / len(successful_queries)
        ...
        return {
            ...
            "errorQueries": len([q for q in self.query_history if q.get("status") == "error"]),
